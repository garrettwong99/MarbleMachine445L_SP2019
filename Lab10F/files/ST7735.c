...
// Daniel Valvano, Takuma Fujiwara
// January 15, 2020
...
// EE445L lab assignment
  ...
  for(uint32_t i=0; i<num; i++){
    if(bufY[i]<Ymin) newY=Ymin;
    if(bufY[i]>Ymax) newY=Ymax;
    if(bufX[i]<MinX) newX=MinX;
    if(bufX[i]>MaxX) newX=MaxX;
    // X goes from 0 to 127
    // j goes from 159 to 32
    // y=Ymax maps to j=32
    // y=Ymin maps to j=159
    newY = 32+(127*(Ymax-bufY[i]))/Yrange;
    newX = (127*((-MinX)+bufX[i]))/RangeX;
    ST7735_DrawPixel(newX,   newY,   color);
    ST7735_DrawPixel(newX+1, newY,   color);
    ST7735_DrawPixel(newX,   newY+1, color);
    ST7735_DrawPixel(newX+1, newY+1, color);
  }
}
